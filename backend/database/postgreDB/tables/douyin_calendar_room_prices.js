...
const createCommentQueryStrings = [
  `COMMENT ON TABLE ${tableName} IS '抖音套餐按日价格表；日历房和预售券各自通过独立接口维护，每个套餐每天只有一条价格';`,
  `COMMENT ON COLUMN ${tableName}.rate_plan_id IS '关联本地售卖套餐 rate_plans.id；仅 CALENDAR_ROOM 或 PRESALE 套餐可由各自接口维护';`,
  `COMMENT ON COLUMN ${tableName}.stay_date IS '房晚日期；DATE 按 YYYY-MM-DD 自然日使用';`,
  `COMMENT ON COLUMN ${tableName}.original_amount IS '套餐实际售价，单位元；调用抖音房价接口时转换为分';`,
  `COMMENT ON COLUMN ${tableName}.retail_amount IS '套餐划线价，单位元；为空时不传给抖音，存在时不得低于实际售价';`,
