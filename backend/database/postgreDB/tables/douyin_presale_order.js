 ...
 * 抖音预售券本地订单主表表名。
 */
const tableName = "douyin_presale_orders";

/**
 * 创建抖音预售券本地订单主表。
 * 说明：
 * 1. 本表用于承接 biz_type=2011 的预售券创单结果；
 * 2. 只保存预售券主订单，不拆分成实际入住日订单；
 * 3. 后续预约单（biz_type=2012）可通过 source_order_id 关联该表。
 */
...
const createCommentQueryStrings = [
  `COMMENT ON TABLE ${tableName} IS '抖音酒店预售券本地订单主表';`,
  `COMMENT ON COLUMN ${tableName}.order_id IS '第三方订单号，回传给抖音的 order_out_id';`,
  `COMMENT ON COLUMN ${tableName}.ota_order_id IS '抖音预售订单号';`,
  `COMMENT ON COLUMN ${tableName}.source_order_id IS '来源预售订单号，供后续预约单关联';`,
  `COMMENT ON COLUMN ${tableName}.biz_type IS '抖音业务类型，预售券固定为 2011';`,
  `COMMENT ON COLUMN ${tableName}.order_stage IS '预售订单阶段';`,
  `COMMENT ON COLUMN ${tableName}.pre_sale_coupon_id IS '预售券ID';`,
  `COMMENT ON COLUMN ${tableName}.goods_id IS '抖音商品ID';`,
  `COMMENT ON COLUMN ${tableName}.sku_id IS '抖音商品SKU ID';`,
  `COMMENT ON COLUMN ${tableName}.rate_plan_id IS '抖音售卖房型ID';`,
  `COMMENT ON COLUMN ${tableName}.voucher_count IS '预售券数量';`,
