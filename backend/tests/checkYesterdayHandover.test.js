...
describe('GET /api/handover/check-yesterday - 检查昨日交接记录', () => {
  ...
  describe('成功场景', () => {
    it('应该正确检查昨日有完整交接记录（4种支付方式）', async () => {
      ...
      // 测试：检查2024-01-16的前一天（2024-01-15）
      const checkDate = '2024-01-16';
    ...
    it('应该正确检查昨日交接记录不完整（只有部分支付方式）', async () => {
      ...
      // 测试：检查2024-01-21的前一天（2024-01-20）
      const checkDate = '2024-01-21';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });
    ...
    it('应该正确检查昨日无交接记录', async () => {
      // 不插入任何数据，直接查询
      const checkDate = '2024-01-25';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });
    ...
    it('应该正确处理跨月份的日期（月初查询上月末）', async () => {
      ...
      // 测试：检查2024-02-01的前一天（2024-01-31）
      const checkDate = '2024-02-01';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });
    ...
    it('应该正确处理跨年份的日期（年初查询上年末）', async () => {
      ...
      // 测试：检查2024-01-01的前一天（2023-12-31）
      const checkDate = '2024-01-01';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });
  ...
  describe('边界情况', () => {
    ...
    it('应该忽略非标准支付方式（payment_type不在1-4范围内）', async () => {
      ...
      // 测试：应该只计算payment_type为1-4的记录
      const checkDate = '2024-04-21';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });
  ...
  describe('日期计算', () => {
    it('应该正确计算2月28日的前一天（非闰年）', async () => {
      ...
      const checkDate = '2023-02-28';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });

      expect(res.status).toBe(200);
      expect(res.body.data.date).toBe('2023-02-27');
      expect(res.body.data.isComplete).toBe(true);
    });

    it('应该正确计算2月29日的前一天（闰年）', async () => {
      ...
      const checkDate = '2024-02-29';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });

      expect(res.status).toBe(200);
      expect(res.body.data.date).toBe('2024-02-28');
      expect(res.body.data.isComplete).toBe(true);
    });

    it('应该正确计算3月1日的前一天（闰年，2月29日）', async () => {
      ...
      const checkDate = '2024-03-01';
      const res = await request(app)
        .get('/api/handover/check-yesterday')
        .query({ date: checkDate });

      expect(res.status).toBe(200);
      expect(res.body.data.date).toBe('2024-02-29');
      expect(res.body.data.isComplete).toBe(true);
    });
  });
