...
describe('房间状态 API - 单日换房后旧房应释放', () => {
  ...
  test('多日订单单日换房后，查询该日房态应以 stay_date 为准', async () => {
    ...
    const res17 = await authedRequest().get('/api/rooms').query({ date: '2025-12-17' });
    ...
    // 同一订单其他日期仍占用原房间 110
    const res16 = await authedRequest().get('/api/rooms').query({ date: '2025-12-16' });
    ...
    const res18 = await authedRequest().get('/api/rooms').query({ date: '2025-12-18' });
