...
const { MAX_ARIS_PER_REQUEST, MAX_BATCH_DAYS, buildDateList } = require('./calendarPrice.validator');
...
function buildBatches(ratePlanId, prices) {
  ...
  for (let index = 0; index < prices.length; index += MAX_BATCH_DAYS) {
    ...
    if (batchAris.length > MAX_ARIS_PER_REQUEST) {
      throw createServiceError('单次房价推送超过 50 组', 400);
    }
...
async function savePushLog(record) {
  try {
    ...
    console.warn('[Douyin Calendar Price] 本地保存 logid 失败:', error.message);
  }
}

/** 将套餐指定日期的房价推送到抖音。 */
async function syncPrices(ratePlanId, options = {}) {
  ...
  if (missingDate) throw createServiceError(`缺少 ${missingDate} 的日历房价格`, 400);
  ...
  for (const aris of buildBatches(context.douyin_rate_plan_id, dates.map((date) => priceMap.get(date)))) {
    ...
    try {
      ...
      throw createServiceError('调用抖音日历房价接口失败', 502);
    }

    const result = await response.json();
    const logId = result?.extra?.logid || result?.extra?.log_id || null;
    if (logId) {
      console.log(`[Douyin Calendar Price] 抖音 logid: ${logId}`);
      logIds.push(logId);
    }
    ...
    if (!response.ok || Number(result?.extra?.error_code || 0) !== 0) {
      await savePushLog({ type: 'calendar_price_push', stage: 'error', logId, ratePlanId, dates: aris.flatMap((ari) => ari.dates), error: errorMessage || `HTTP ${response.status}` });
    ...
    if (!saveResult || Number(saveResult.code) !== 0) {
      await savePushLog({ type: 'calendar_price_push', stage: 'error', logId, ratePlanId, dates: aris.flatMap((ari) => ari.dates), error: saveResult?.message || '抖音房价推送失败' });
    ...
    await savePushLog({ type: 'calendar_price_push', stage: 'processed', logId, ratePlanId, douyinRatePlanId: context.douyin_rate_plan_id, dates: batchDates });
