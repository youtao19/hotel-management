 ...
 */
function buildDouyinRefundResultResponse({ errorCode, description }) {
  return {
    data: {
 ...
 */
async function receiveDouyinRefundResultCallback(req, res) {
  const requestLogId = resolveDouyinRequestLogId(req)

  try {
    const result = await handleDouyinRefundResult(req.body || {}, {
      douyinLogId: requestLogId,
    })

    return res.json(buildDouyinRefundResultResponse({
...
module.exports = {
  buildDouyinRefundResultResponse,
