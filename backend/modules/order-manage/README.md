...
`order-manage` 负责订单列表、订单详情、订单编辑、订单状态变更、每日换房、退房、提前退房和押金退款相关接口。
...
- Phase 3 目前已移动只读 SQL、提前退房建议读取逻辑、基础编辑 SQL、联合订单账单编辑 SQL、押金状态 SQL、订单状态更新 SQL、每日换房 SQL、退押账单创建流程、正常退房事务和提前退房事务。
